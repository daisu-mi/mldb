...
class MleagueDB(object):
    ...
    def openScoreOffline(self):
        self.fp = open(self.localfile, 'r', encoding="utf-8")
...
mldb = MleagueDB('','teams.csv','players.csv')

#mldb.openScoreOnline()
#mldb.downloadScore()
#mldb_player = mldb.getPlayers()

mldb.openScoreOffline()
mldb.readScore()
mldb.closeScore()

mldb_player = mldb.getPlayers()
mldb_result = mldb.getResults()

first_sheet = 1

#for i in ['1']:
for i in mldb_player.keys():
    i = int(i)
    player_name = mldb.getPlayerName(i)

    df_output = mldb.analyzeScore(i)
